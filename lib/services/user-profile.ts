...
import type { StepProgress, StepStatus } from "@/lib/form-validators/form-steps"
import { ProfileErrorMessages, getAdminFeedbackError } from "@/lib/types/error-messages"
...
export const submitStepForReview = async (userId: string, stepId: string) => {
  try {
    const userProfileRef = doc(db, "user-profiles", userId)
    const docSnap = await getDoc(userProfileRef)
...
export const getNextIncompleteStep = (profile: UserProfile, availableSteps: string[]): string | null => {
...
export const canProceedToNextStep = (profile: UserProfile, currentStepId: string, nextStepId: string): boolean => {
...
export const getStepValidationErrors = (profile: UserProfile, stepId: string): string[] => {
  const stepProgress = profile.stepProgress[stepId]
  const errors: string[] = []
